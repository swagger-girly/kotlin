...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw HelloWorldTestinggggInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw HelloWorldTestinggggInvalidDataException("`$name` is not set")
            is JsonNull -> throw HelloWorldTestinggggInvalidDataException("`$name` is null")
            else ->
                throw HelloWorldTestinggggInvalidDataException("`$name` is invalid, received $this")
        }
