...
private constructor(
    private val listed: StoreActivityListed? = null,
    private val sold: StoreActivitySold? = null,
    private val archived: StoreActivityArchived? = null,
    private val transferred: StoreActivityTransferred? = null,
     ...
     */
    fun listed(): StoreActivityListed? = listed
     ...
     */
    fun sold(): StoreActivitySold? = sold
     ...
     * the heritage clause has to omit the redeclared key instead of extending the base plainly.
     */
    fun asListed(): StoreActivityListed = listed.getOrThrow("listed")

    /**
     * Sale activity. Pins the base's `status` to a single member — a provable narrowing, so this
     * arm keeps a plain `extends` with no omitted keys.
     */
    fun asSold(): StoreActivitySold = sold.getOrThrow("sold")

    /**
     * Archival activity. Narrows the base's `status` by value but also makes it nullable, which
     * `Pet` does not accept — a value subset alone is not enough to keep a plain `extends` here.
     */
    fun asArchived(): StoreActivityArchived = archived.getOrThrow("archived")

    /**
     * Transfer activity. Reaches `Pet` only through the pass-through wrapper branch, so the base is
     * resolvable only transitively.
     */
    fun asTransferred(): StoreActivityTransferred = transferred.getOrThrow("transferred")
     ...
     *     override fun visitListed(listed: StoreActivityListed): String? = listed.toString()
     *
     *     // ...
     *
    ...
    fun validate(): StoreRetrieveActivityResponse = apply {
        ...
        accept(
            object : Visitor<Unit> {
                override fun visitListed(listed: StoreActivityListed) {
                    listed.validate()
                }

                override fun visitSold(sold: StoreActivitySold) {
                    sold.validate()
                }

                override fun visitArchived(archived: StoreActivityArchived) {
                    archived.validate()
                }

                override fun visitTransferred(transferred: StoreActivityTransferred) {
    ...
    internal fun validity(): Int =
        accept(
            object : Visitor<Int> {
                override fun visitListed(listed: StoreActivityListed) = listed.validity()

                override fun visitSold(sold: StoreActivitySold) = sold.validity()

                override fun visitArchived(archived: StoreActivityArchived) = archived.validity()

                override fun visitTransferred(transferred: StoreActivityTransferred) =
                    transferred.validity()
    ...
    companion object {
         ...
         * plainly.
         */
        fun ofListed(listed: StoreActivityListed) = StoreRetrieveActivityResponse(listed = listed)
         ...
         */
        fun ofSold(sold: StoreActivitySold) = StoreRetrieveActivityResponse(sold = sold)
         ...
         * here.
         */
        fun ofArchived(archived: StoreActivityArchived) =
            StoreRetrieveActivityResponse(archived = archived)

        /**
         * Transfer activity. Reaches `Pet` only through the pass-through wrapper branch, so the
         * base is resolvable only transitively.
         */
        fun ofTransferred(transferred: StoreActivityTransferred) =
    ...
    interface Visitor<out T> {
         ...
         * plainly.
         */
        fun visitListed(listed: StoreActivityListed): T

        /**
         * Sale activity. Pins the base's `status` to a single member — a provable narrowing, so
         * this arm keeps a plain `extends` with no omitted keys.
         */
        fun visitSold(sold: StoreActivitySold): T
         ...
         * here.
         */
        fun visitArchived(archived: StoreActivityArchived): T

        /**
         * Transfer activity. Reaches `Pet` only through the pass-through wrapper branch, so the
         * base is resolvable only transitively.
         */
        fun visitTransferred(transferred: StoreActivityTransferred): T
    ...
    internal class Deserializer :
        ...
        override fun ObjectCodec.deserialize(node: JsonNode): StoreRetrieveActivityResponse {
            ...
            when (family) {
                "listed" -> {
                    return tryDeserialize(node, jacksonTypeRef<StoreActivityListed>())?.let {
                        StoreRetrieveActivityResponse(listed = it, _json = json)
                    } ?: StoreRetrieveActivityResponse(_json = json)
                }
                "sold" -> {
                    return tryDeserialize(node, jacksonTypeRef<StoreActivitySold>())?.let {
                        StoreRetrieveActivityResponse(sold = it, _json = json)
                    } ?: StoreRetrieveActivityResponse(_json = json)
                }
                "archived" -> {
                    return tryDeserialize(node, jacksonTypeRef<StoreActivityArchived>())?.let {
                        StoreRetrieveActivityResponse(archived = it, _json = json)
                    } ?: StoreRetrieveActivityResponse(_json = json)
                }
                "transferred" -> {
                    return tryDeserialize(node, jacksonTypeRef<StoreActivityTransferred>())?.let {
    ...
    private constructor(
         ...
         * ```kotlin
         * val myObject: MyClass = storeActivityListed.category().convert(MyClass::class.java)
         * ```
         */
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityListed.relatedCategory().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedCategory")
        @ExcludeMissing
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityListed.relatedCustomer().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedCustomer")
        @ExcludeMissing
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityListed.relatedOrder().convert(MyClass::class.java)
         * ```
         */
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityListed.relatedShelter().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedShelter")
        @ExcludeMissing
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityListed.relatedUser().convert(MyClass::class.java)
         * ```
         */
        ...
        companion object {
            ...
            /**
             * Returns a mutable builder for constructing an instance of [StoreActivityListed].
             *
        ...
        /** A builder for [StoreActivityListed]. */
        class Builder internal constructor() {
            ...
            internal fun from(storeActivityListed: StoreActivityListed) = apply {
                name = storeActivityListed.name
                photoUrls = storeActivityListed.photoUrls.map { it.toMutableList() }
                id = storeActivityListed.id
                acquisitionChannel = storeActivityListed.acquisitionChannel
                category = storeActivityListed.category
                microchipId = storeActivityListed.microchipId
                relatedAddress = storeActivityListed.relatedAddress
                relatedCategory = storeActivityListed.relatedCategory
                relatedCustomer = storeActivityListed.relatedCustomer
                relatedMoney = storeActivityListed.relatedMoney
                relatedOrder = storeActivityListed.relatedOrder
                relatedPet = storeActivityListed.relatedPet
                relatedShelter = storeActivityListed.relatedShelter
                relatedUser = storeActivityListed.relatedUser
                status = storeActivityListed.status
                tags = storeActivityListed.tags.map { it.toMutableList() }
                activityId = storeActivityListed.activityId
                family = storeActivityListed.family
                recordedAt = storeActivityListed.recordedAt
                additionalProperties = storeActivityListed.additionalProperties.toMutableMap()
            }
            ...
            /**
             * Returns an immutable instance of [StoreActivityListed].
             *
         ...
         *   match its expected type.
         */
        fun validate(): StoreActivityListed = apply {
            if (validated) {
                return@apply
            }
        ...
        override fun equals(other: Any?): Boolean {
            ...
            return other is StoreActivityListed &&
                name == other.name &&
    ...
    private constructor(
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivitySold.category().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("category") @ExcludeMissing fun _category(): JsonValue = category
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivitySold.relatedCategory().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedCategory")
        @ExcludeMissing
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivitySold.relatedCustomer().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedCustomer")
        @ExcludeMissing
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivitySold.relatedOrder().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedOrder") @ExcludeMissing fun _relatedOrder(): JsonValue = relatedOrder
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivitySold.relatedShelter().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedShelter")
        @ExcludeMissing
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivitySold.relatedUser().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedUser") @ExcludeMissing fun _relatedUser(): JsonValue = relatedUser
        ...
        companion object {
            ...
            /**
             * Returns a mutable builder for constructing an instance of [StoreActivitySold].
             *
             * The following fields are required:
             * ```kotlin
             * .name()
        ...
        /** A builder for [StoreActivitySold]. */
        class Builder internal constructor() {
            ...
            internal fun from(storeActivitySold: StoreActivitySold) = apply {
                name = storeActivitySold.name
                photoUrls = storeActivitySold.photoUrls.map { it.toMutableList() }
                id = storeActivitySold.id
                acquisitionChannel = storeActivitySold.acquisitionChannel
                category = storeActivitySold.category
                microchipId = storeActivitySold.microchipId
                relatedAddress = storeActivitySold.relatedAddress
                relatedCategory = storeActivitySold.relatedCategory
                relatedCustomer = storeActivitySold.relatedCustomer
                relatedMoney = storeActivitySold.relatedMoney
                relatedOrder = storeActivitySold.relatedOrder
                relatedPet = storeActivitySold.relatedPet
                relatedShelter = storeActivitySold.relatedShelter
                relatedUser = storeActivitySold.relatedUser
                status = storeActivitySold.status
                tags = storeActivitySold.tags.map { it.toMutableList() }
                activityId = storeActivitySold.activityId
                family = storeActivitySold.family
                recordedAt = storeActivitySold.recordedAt
                additionalProperties = storeActivitySold.additionalProperties.toMutableMap()
            }
            ...
            /**
             * Returns an immutable instance of [StoreActivitySold].
             *
             * Further updates to this [Builder] will not mutate the returned instance.
             *
             * The following fields are required:
         ...
         *   match its expected type.
         */
        fun validate(): StoreActivitySold = apply {
            if (validated) {
                return@apply
            }
        ...
        override fun equals(other: Any?): Boolean {
            ...
            return other is StoreActivitySold &&
                name == other.name &&
                photoUrls == other.photoUrls &&
                id == other.id &&
                acquisitionChannel == other.acquisitionChannel &&
        ...
        override fun toString() =
            "StoreActivitySold{name=$name, photoUrls=$photoUrls, id=$id, acquisitionChannel=$acquisitionChannel, category=$category, microchipId=$microchipId, relatedAddress=$relatedAddress, relatedCategory=$relatedCategory, relatedCustomer=$relatedCustomer, relatedMoney=$relatedMoney, relatedOrder=$relatedOrder, relatedPet=$relatedPet, relatedShelter=$relatedShelter, relatedUser=$relatedUser, status=$status, tags=$tags, activityId=$activityId, family=$family, recordedAt=$recordedAt, additionalProperties=$additionalProperties}"
    }
    ...
    private constructor(
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityArchived.category().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("category") @ExcludeMissing fun _category(): JsonValue = category
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityArchived.relatedCategory().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedCategory")
        @ExcludeMissing
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityArchived.relatedCustomer().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedCustomer")
        @ExcludeMissing
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityArchived.relatedOrder().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedOrder") @ExcludeMissing fun _relatedOrder(): JsonValue = relatedOrder
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityArchived.relatedShelter().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedShelter")
        @ExcludeMissing
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityArchived.relatedUser().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedUser") @ExcludeMissing fun _relatedUser(): JsonValue = relatedUser
        ...
        companion object {
            ...
            /**
             * Returns a mutable builder for constructing an instance of [StoreActivityArchived].
             *
             * The following fields are required:
             * ```kotlin
             * .name()
        ...
        /** A builder for [StoreActivityArchived]. */
        class Builder internal constructor() {
            ...
            internal fun from(storeActivityArchived: StoreActivityArchived) = apply {
                name = storeActivityArchived.name
                photoUrls = storeActivityArchived.photoUrls.map { it.toMutableList() }
                id = storeActivityArchived.id
                acquisitionChannel = storeActivityArchived.acquisitionChannel
                category = storeActivityArchived.category
                microchipId = storeActivityArchived.microchipId
                relatedAddress = storeActivityArchived.relatedAddress
                relatedCategory = storeActivityArchived.relatedCategory
                relatedCustomer = storeActivityArchived.relatedCustomer
                relatedMoney = storeActivityArchived.relatedMoney
                relatedOrder = storeActivityArchived.relatedOrder
                relatedPet = storeActivityArchived.relatedPet
                relatedShelter = storeActivityArchived.relatedShelter
                relatedUser = storeActivityArchived.relatedUser
                status = storeActivityArchived.status
                tags = storeActivityArchived.tags.map { it.toMutableList() }
                activityId = storeActivityArchived.activityId
                family = storeActivityArchived.family
                recordedAt = storeActivityArchived.recordedAt
                archivedReason = storeActivityArchived.archivedReason
                additionalProperties = storeActivityArchived.additionalProperties.toMutableMap()
            }
            ...
            /**
             * Returns an immutable instance of [StoreActivityArchived].
             *
             * Further updates to this [Builder] will not mutate the returned instance.
             *
             * The following fields are required:
             ...
             * @throws IllegalStateException if any required field is unset.
             */
            fun build(): StoreActivityArchived =
                StoreActivityArchived(
                    checkRequired("name", name),
                    checkRequired("photoUrls", photoUrls).map { it.toImmutable() },
                    id,
                    acquisitionChannel,
         ...
         *   match its expected type.
         */
        fun validate(): StoreActivityArchived = apply {
            if (validated) {
                return@apply
            }
        ...
        override fun equals(other: Any?): Boolean {
            ...
            return other is StoreActivityArchived &&
                name == other.name &&
                photoUrls == other.photoUrls &&
                id == other.id &&
                acquisitionChannel == other.acquisitionChannel &&
        ...
        override fun toString() =
            "StoreActivityArchived{name=$name, photoUrls=$photoUrls, id=$id, acquisitionChannel=$acquisitionChannel, category=$category, microchipId=$microchipId, relatedAddress=$relatedAddress, relatedCategory=$relatedCategory, relatedCustomer=$relatedCustomer, relatedMoney=$relatedMoney, relatedOrder=$relatedOrder, relatedPet=$relatedPet, relatedShelter=$relatedShelter, relatedUser=$relatedUser, status=$status, tags=$tags, activityId=$activityId, family=$family, recordedAt=$recordedAt, archivedReason=$archivedReason, additionalProperties=$additionalProperties}"
    }
     ...
     * resolvable only transitively.
     */
    class StoreActivityTransferred
    @JsonCreator(mode = JsonCreator.Mode.DISABLED)
    private constructor(
        private val name: JsonField<String>,
        private val photoUrls: JsonField<List<String>>,
        ...
        private val tags: JsonField<List<JsonValue>>,
        private val family: JsonField<Family>,
        private val transferredTo: JsonField<Shelter>,
        private val additionalProperties: MutableMap<String, JsonValue>,
    ) {
        ...
        private constructor(
            ...
            @ExcludeMissing
            transferredTo: JsonField<Shelter> = JsonMissing.of(),
        ) : this(
            name,
            photoUrls,
            id,
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityTransferred.category().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("category") @ExcludeMissing fun _category(): JsonValue = category
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityTransferred.relatedCategory().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedCategory")
        @ExcludeMissing
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityTransferred.relatedCustomer().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedCustomer")
        @ExcludeMissing
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityTransferred.relatedOrder().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedOrder") @ExcludeMissing fun _relatedOrder(): JsonValue = relatedOrder
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityTransferred.relatedShelter().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedShelter")
        @ExcludeMissing
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```kotlin
         * val myObject: MyClass = storeActivityTransferred.relatedUser().convert(MyClass::class.java)
         * ```
         */
        @JsonProperty("relatedUser") @ExcludeMissing fun _relatedUser(): JsonValue = relatedUser
         ...
         *   (e.g. if the server responded with an unexpected value).
         */
        fun transferredTo(): Shelter? = transferredTo.getNullable("transferredTo")
        ...
        @JsonProperty("transferredTo")
        @ExcludeMissing
        fun _transferredTo(): JsonField<Shelter> = transferredTo
        ...
        companion object {
            ...
            /**
             * Returns a mutable builder for constructing an instance of [StoreActivityTransferred].
             *
             * The following fields are required:
             * ```kotlin
             * .name()
        ...
        /** A builder for [StoreActivityTransferred]. */
        class Builder internal constructor() {
            ...
            private var tags: JsonField<MutableList<JsonValue>>? = null
            private var family: JsonField<Family>? = null
            private var transferredTo: JsonField<Shelter> = JsonMissing.of()
            private var additionalProperties: MutableMap<String, JsonValue> = mutableMapOf()

            internal fun from(storeActivityTransferred: StoreActivityTransferred) = apply {
                name = storeActivityTransferred.name
                photoUrls = storeActivityTransferred.photoUrls.map { it.toMutableList() }
                id = storeActivityTransferred.id
                acquisitionChannel = storeActivityTransferred.acquisitionChannel
                category = storeActivityTransferred.category
                microchipId = storeActivityTransferred.microchipId
                relatedAddress = storeActivityTransferred.relatedAddress
                relatedCategory = storeActivityTransferred.relatedCategory
                relatedCustomer = storeActivityTransferred.relatedCustomer
                relatedMoney = storeActivityTransferred.relatedMoney
                relatedOrder = storeActivityTransferred.relatedOrder
                relatedPet = storeActivityTransferred.relatedPet
                relatedShelter = storeActivityTransferred.relatedShelter
                relatedUser = storeActivityTransferred.relatedUser
                status = storeActivityTransferred.status
                tags = storeActivityTransferred.tags.map { it.toMutableList() }
                family = storeActivityTransferred.family
                transferredTo = storeActivityTransferred.transferredTo
                additionalProperties = storeActivityTransferred.additionalProperties.toMutableMap()
            }
            ...
            fun transferredTo(transferredTo: Shelter) = transferredTo(JsonField.of(transferredTo))

            /**
             * Sets [Builder.transferredTo] to an arbitrary JSON value.
             *
             * You should usually call [Builder.transferredTo] with a well-typed [Shelter] value
             * instead. This method is primarily for setting the field to an undocumented or not yet
             * supported value.
             */
            fun transferredTo(transferredTo: JsonField<Shelter>) = apply {
            ...
            /**
             * Returns an immutable instance of [StoreActivityTransferred].
             *
             * Further updates to this [Builder] will not mutate the returned instance.
             *
             * The following fields are required:
             ...
             * @throws IllegalStateException if any required field is unset.
             */
            fun build(): StoreActivityTransferred =
         ...
         *   match its expected type.
         */
        fun validate(): StoreActivityTransferred = apply {
            if (validated) {
                return@apply
            }
        ...
        override fun equals(other: Any?): Boolean {
            ...
            return other is StoreActivityTransferred &&
                name == other.name &&
                photoUrls == other.photoUrls &&
                id == other.id &&
                acquisitionChannel == other.acquisitionChannel &&
        ...
        override fun toString() =
            "StoreActivityTransferred{name=$name, photoUrls=$photoUrls, id=$id, acquisitionChannel=$acquisitionChannel, category=$category, microchipId=$microchipId, relatedAddress=$relatedAddress, relatedCategory=$relatedCategory, relatedCustomer=$relatedCustomer, relatedMoney=$relatedMoney, relatedOrder=$relatedOrder, relatedPet=$relatedPet, relatedShelter=$relatedShelter, relatedUser=$relatedUser, status=$status, tags=$tags, family=$family, transferredTo=$transferredTo, additionalProperties=$additionalProperties}"
    }
}
