...
private constructor(
    private val approved: PetModerationApprovedEvent? = null,
    private val rejected: PetModerationRejectedEvent? = null,
    private val _json: JsonValue? = null,
) {

    fun approved(): PetModerationApprovedEvent? = approved

    fun rejected(): PetModerationRejectedEvent? = rejected

    fun isApproved(): Boolean = approved != null

    fun isRejected(): Boolean = rejected != null

    fun asApproved(): PetModerationApprovedEvent = approved.getOrThrow("approved")

    fun asRejected(): PetModerationRejectedEvent = rejected.getOrThrow("rejected")
     ...
     *     override fun visitApproved(approved: PetModerationApprovedEvent): String? = approved.toString()
     *
     *     // ...
     *
    ...
    fun validate(): PetModerationWebhookEvent = apply {
        ...
        accept(
            object : Visitor<Unit> {
                override fun visitApproved(approved: PetModerationApprovedEvent) {
                    approved.validate()
                }

                override fun visitRejected(rejected: PetModerationRejectedEvent) {
                    rejected.validate()
                }
            }
        )
    ...
    companion object {
        ...
        fun ofApproved(approved: PetModerationApprovedEvent) =
            PetModerationWebhookEvent(approved = approved)

        fun ofRejected(rejected: PetModerationRejectedEvent) =
    ...
    interface Visitor<out T> {
        ...
        fun visitApproved(approved: PetModerationApprovedEvent): T

        fun visitRejected(rejected: PetModerationRejectedEvent): T
        ...
        override fun ObjectCodec.deserialize(node: JsonNode): PetModerationWebhookEvent {
            ...
            when (type) {
                "pet.moderation.approved" -> {
                    return tryDeserialize(node, jacksonTypeRef<PetModerationApprovedEvent>())?.let {
                        PetModerationWebhookEvent(approved = it, _json = json)
                    } ?: PetModerationWebhookEvent(_json = json)
                }
                "pet.moderation.rejected" -> {
                    return tryDeserialize(node, jacksonTypeRef<PetModerationRejectedEvent>())?.let {
